//! [POST /_synapse/admin/v2/users/:user_id/devices](https://github.com/element-hq/synapse/blob/master/docs/admin_api/user_admin_api.md#create-a-device)

use ruma::{
    OwnedDeviceId, OwnedUserId,
    api::{auth_scheme::AccessToken, metadata, request, response},
};

metadata! {
    method: POST,
    rate_limited: false,
    authentication: AccessToken,
    path: "/_synapse/admin/v2/users/{user_id}/devices",
}

#[request]
pub struct Request {
    /// The user to create the device for.
    #[ruma_api(path)]
    pub user_id: OwnedUserId,

    /// The ID of the device to create.
    ///
    /// Creating a device that already exists is a no-op.
    pub device_id: OwnedDeviceId,
}

#[derive(Default)]
#[response]
pub struct Response {}

impl Request {
    /// Creates a `Request` with the given user ID and device ID.
    pub fn new(user_id: OwnedUserId, device_id: OwnedDeviceId) -> Self {
        Self { user_id, device_id }
    }
}

impl Response {
    /// Creates an empty `Response`.
    pub fn new() -> Self {
        Self {}
    }
}
